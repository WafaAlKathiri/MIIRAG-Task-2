...
The only issue is that the synthetic road crack images are always generated with a while border arond the edges, so this method also attempts to remove them after generation and before blending. As can be seen in the image, the method worked with some cracks but not all, two in the bottom left seem to be blended quite well, but the rest need imrovement.  
